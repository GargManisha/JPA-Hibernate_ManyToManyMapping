package com.hibernate.crud.advanceMappingManyToMany;
...
import com.hibernate.crud.advanceMappingManyToMany.DAO.DAO;
...
import com.hibernate.crud.advanceMappingManyToMany.entity.Review;
import com.hibernate.crud.advanceMappingManyToMany.entity.Student;

@SpringBootApplication
public class AdvanceMappingManyToManyApplication {

	public static void main(String[] args) {
		SpringApplication.run(AdvanceMappingManyToManyApplication.class, args);
	}

	@Bean
	public CommandLineRunner commandLineRunner(DAO dao) {
		return runner -> {
			//following methods for many-to-many mapping (Bi-directional)
			//createCoursesAndStudents(dao);
			//findCourseAndStudentByCourseId(dao);
			//findStudentAndCourseByStudentId(dao);
	...
	private void deleteStudent(DAO dao) {
	...
	private void addMoreCoursesForStudent(DAO dao) {

		int id = 7;
		
		Student tempStudent = dao.findStudentAndCourseByStudentId(id);
	...
	private void findStudentAndCourseByStudentId(DAO dao) {
	...
	private void findCourseAndStudentByCourseId(DAO dao) {

		int id = 4;
		
		System.out.println("Finding course Id: " + id);
		Course tempCourse = dao.findCourseAndStudentByCourseId(id);
		
		System.out.println("Course data: " + tempCourse);
		System.out.println("Student data: " + tempCourse.getStudents());
		System.out.println("Done!");
	}

	private void createCoursesAndStudents(DAO dao) {

		Course tempCourse = new Course("Pacman - How to score 1 million points.");
		
		Student tempStudent1 = new Student("Join","Doe","john.doe@gmail.com");
		Student tempStudent2 = new Student("Marry","Public","marry@gmail.com");
		
		tempCourse.addStudent(tempStudent1);
		tempCourse.addStudent(tempStudent2);
		
		System.out.println("Saving course and student");
